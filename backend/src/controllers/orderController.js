...
const hydrateOrdersWithInventoryQrCodes = async (orders = [], options = {}) => {
  ...
  // Product serial arrays can grow large.  Return only the serial units that
  // belong to these orders rather than loading every unit stored on a product.
  const serialUnitMatches = [];
  if (serialNumbers.length > 0) {
    serialUnitMatches.push({ $in: ["$$unit.serialNumber", serialNumbers] });
  }
  if (orderCodes.length > 0) {
    serialUnitMatches.push({ $in: ["$$unit.assignedOrderCode", orderCodes] });
  }
  if (orderIds.length > 0) {
    serialUnitMatches.push({ $in: ["$$unit.assignedOrderId", orderIds] });
  }
...
const listOrdersForAdmin = async (req, res) => {
  ...
  // duplicate task lookup here keeps simultaneous admin sessions responsive.
  const hydratedOrders = await hydrateOrdersWithInventoryQrCodes(orders, {
